...
#
# 使用方法:
#   ./collect_data.sh                    # デフォルト: 連続駆動モード, 30回
#   ./collect_data.sh -n 10 -d reboot    # 再起動実験, 10回
#   ./collect_data.sh -n 30 -i 30 -d continuous  # 30秒カウントダウン付き
#
# ===========================================================================
set -euo pipefail
...
EXAMPLES:
  $(basename "$0")                       # 連続駆動, 30回, 30秒間隔
  $(basename "$0") -n 5 -d reboot       # 再起動実験, 5回
...
# 引数パース
# -------------------------------------------------------------------------
while getopts "n:i:d:s:h" opt; do
    case "$opt" in
...
case "$DATA_MODE" in
    continuous)
        OUTPUT_DIR="${PROJECT_ROOT}/data/run_continuous"
        ;;
    reboot)
        OUTPUT_DIR="${PROJECT_ROOT}/data/run_reboot"
        ;;
    *)
        echo "[ERROR] 不明なデータモード: ${DATA_MODE} (continuous/reboot を指定してください)"
        exit 1
        ;;
esac
...
for i in $(seq 1 "$NUM_MEASUREMENTS"); do
    ...
    if wait_for_result; then
        ts=$(timestamp)
        if [ "$DATA_MODE" = "reboot" ]; then
            dest_name="hp_reboot${i}_${ts}.txt"
        else
...
echo "次のステップ:"
echo "  cd ${SCRIPT_DIR}"
echo "  python analyzer.py ${OUTPUT_DIR}"
echo "  python plotter.py  ${OUTPUT_DIR} --output ${PROJECT_ROOT}/figures"
echo ""
